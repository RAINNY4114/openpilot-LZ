...
def has_systemd_dev() -> bool:
  if arch == "Darwin":
    return False

  try:
    if subprocess.call(["pkg-config", "--exists", "libsystemd"]) == 0:
      return True
  except Exception:
    pass
...
systemd_available = has_systemd_dev()
...
qt_env = env.Clone()
qt_base_modules = ["Widgets", "Gui", "Core", "Network", "Concurrent", "DBus", "Xml"]
qt_map_modules = ["Qml", "QuickWidgets", "Location", "Positioning"]
qt_modules = qt_base_modules if wsl2 else qt_base_modules + qt_map_modules
...
if arch != "Darwin":
  if systemd_available:
    ...
    print("libsystemd development files missing. Skipping logcatd build")

...
class AmapNaviServ:
  def __init__(self):
    ...
    self.sm = messaging.SubMaster(['modelV2', 'selfdriveState', 'radarState'])
  ...
  def start_navi_comm(self):
    ...
    #threading.Thread(target=self._navi_recv_thread, daemon=True).start()
    # 启动清理客户端线程
  ...
  def _navi_recv_thread(self):
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
      sock.settimeout(10)
      sock.bind(('0.0.0.0', self.navi_port))
      print("NAVI receive thread started...")
      while True:
        try:
          data, addr = sock.recvfrom(4096)
          ip, port = addr
